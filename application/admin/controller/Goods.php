...
class Goods extends Base
{
	...
	{
		if($this->otype != 3){
			echo 'error-88052!!';exit;
		}
		$proinfo = Db::name('productinfo')->alias('pi')->field('pi.*,pc.pcname')->join('__PRODUCTCLASS__ pc','pc.pcid = pi.cid')->where('pi.isdelete',0)->order('pi.proorder asc')->select();
...
//	}
    ...
    public function huishou()
    {
        ...
        $proinfo = Db::name('productinfo')->alias('pi')->field('pi.*,pc.pcname')->join('__PRODUCTCLASS__ pc','pc.pcid = pi.cid')->where('pi.isdelete',1)->order('pi.pid asc')->select();
        $this->assign('proinfo',$proinfo);
        return $this->fetch();
    }
