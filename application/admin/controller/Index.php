...
//        thinkcod();
        ...
        $order = Db::name('order')->alias('o')->field('o.*,u.username as username,u.nickname as nickname,u.managername')->join('__USERINFO__ u','o.uid = u.uid')->where($_map)->limit('0,10')->order('oid desc')->select();
    ...
    {
        $pagenum = cache('page');
        $newsinfo = Db::name('newsinfo')->alias('ni')->field('ni.*,nc.fclass as fclass')->join('__NEWSCLASS__ nc','nc.fid = ni.fid')->where('ni.isdelete',0)->order('nid desc')->paginate($pagenum);
    ...
    public function checkNewOrder() {
        $hasNewOrder = false;
        $orderswitch = getconf('orderswitch');
        $orderCount = Db::name('order')->count();
        if($orderswitch == 1){
            if(Cookie::has('orderCount') && $orderCount > Cookie::get('orderCount')) {
                $hasNewOrder = true;
            }
        }
        Cookie::set('orderCount', $orderCount);
        return WPreturn('检查订单',$hasNewOrder);
    }

    /**
     * 检查是否有新的充值提现申请
     */
    public function checkNewCash() {
        $hasNewCash = false;
        $cashswitch = getconf('cashswitch');
        $cashCount = Db::name('balance')->count();
        if($cashswitch == 1){
            if(Cookie::has('cashCount') && $cashCount > Cookie::get('cashCount')) {
                $hasNewCash = true;
            }
        }
        Cookie::set('cashCount', $cashCount);
        return WPreturn('检查提现充值',$hasNewCash);
    }
