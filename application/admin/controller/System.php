...
//	}
    ...
    public function homepic()
    {
        if (input('post.')) {
            ...
            $cn_file = request()->file('cn_img');
            $en_file = request()->file('en_img');
            $ptn_file = request()->file('ptn_img');

            if (!isset($cn_file) || !isset($en_file) || !isset($ptn_file)) {
                $this->error("请上传图片");
            }

            $cn_info = $cn_file->move(ROOT_PATH . 'public' . DS . 'uploads');
            $en_info = $en_file->move(ROOT_PATH . 'public' . DS . 'uploads');
            $ptn_info = $ptn_file->move(ROOT_PATH . 'public' . DS . 'uploads');
            if ($cn_info) {
                $data['cn_img'] = $path . $cn_info->getSaveName();
            } else {
                $this->error('中文图片上传失败，请换图片');
            }

            if ($en_info) {
                $data['en_img'] = $path . $en_info->getSaveName();
            } else {
                $this->error('英文图片上传失败，请换图片');
            }

            if ($ptn_info) {
                $data['pn_img'] = $path . $ptn_info->getSaveName();
            } else {
                $this->error('葡萄牙文图片上传失败，请换图片');
            }
