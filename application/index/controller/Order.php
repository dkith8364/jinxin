...
class Order extends Base
{
	...
	public function addorder()
	{
		...
		*/
		if($data['order_price'] > $conf['order_max_price']){
			return WPreturn(_lang('单笔持仓最大为').$conf['order_max_price'].'！',-1);
		}
		$allcount = Db::name('order')->where(array('ostaus'=>0,'uid'=>$data['uid']))->count();
		if($allcount >  $conf['max_order_count']){
			return WPreturn(_lang('最大持仓数量为').$conf['max_order_count'].'！',-1);
		}
		//验证是否开市
		...
		//验证用户是否被冻结
		if($user['ustatus'] != 0){
			return WPreturn(_lang('您的账户已被冻结！'),-1);
		}
		...
		if($user['usermoney'] < $data['order_price'] + $web_poundage){
			return WPreturn(_lang('您得余额不足，请充值！'),-1);
		}
		//验证金额 20 ~ 5000
		if($data['order_price'] < $conf['order_min_price'] || $data['order_price'] > $conf['order_max_price']){
			return WPreturn(_lang('抱歉！单笔持仓在').$conf['order_min_price'].'~'.$conf['order_max_price']._lang('之间！'),-1);
		}
		//手续费
        ...
        if($ids){
        	...
        	set_price_log($data['uid'],2,$u_fee,_lang('下单'),_lang('下单成功'),$ids,$nowmoney);
        	...
        	if($editmoney){
        		...
        		return WPreturn(_lang('下单失败，请重试！'),-1);
        	}
	...
	public function ajaxorder_list()
	{
		...
		if($hold){
            foreach ($hold as $k =>$v){
                $price = db('productdata')->field('pid,Price')->where(['pid'=>$v['pid']])->value('Price');
                $chaprice = $price - $v['buyprice'];
                if($chaprice>0){
                    $hold[$k]['ploss'] = intval($v['fee']*$v['endloss'])/100;
                }elseif($chaprice<0){
                    $hold[$k]['ploss'] = -intval($v['fee']*$v['endloss'])/100;
                }else{
                    $hold[$k]['ploss'] = $v['fee'];
...
//		foreach ($hold as $k => $v){
//		    if($v['is_win'] == 1){
//                $hold[$k]['closeprice'] = $v.fee*$v.endloss/100;
//            }elseif($v['is_win'] == 2){
//                $hold[$k]['closeprice'] = -$v.endloss*10;
//            }else{
//                $hold[$k]['closeprice'] = 0;
//            }
//
//        }
	...
	public function getchart()
    {
        ...
        $data['jiaoyilishi'] = _lang('交易记录');
        $data['chicangmingxi'] = _lang('持仓明细');
        $data['lishimingxi'] = _lang('历史明细');
