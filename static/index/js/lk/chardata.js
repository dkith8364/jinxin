...
function toggle_order_confirm_panel(type) {
    ...
    if(type == 'lookup'){
        var typename = getCookie('zh_choose')=='t'?'買漲':getCookie('zh_choose')=='e'?'lookup':getCookie('zh_choose')=='pt'?'Comprar':'买涨';
        order_type = 0;

        $('.order_type').removeClass('fall');
        $('.order_type').addClass('rise');
    }else{
        var typename = getCookie('zh_choose')=='t'?'買跌':getCookie('zh_choose')=='e'?'lookdown':getCookie('zh_choose')=='pt'?'Compre ou caia':'买跌';
        order_type = 1;
...
function addorder(){
    ...
    if(order_price > my_money){
        err_info('<?php echo _lang("资金不足，请先充值")?>');
        return;
    }
    if(order_price < order_min_price){
        err_info('<?php echo _lang("最小下注金额为")?>'+order_min_price);
        return;
    }
    if(order_price > order_max_price){
        err_info('<?php echo _lang("最大下注金额为")?>'+order_max_price);
        return;
    }
    ...
    $.post(posturl,postdata,function(resdata){
        ...
        if(resdata.type == 1){
            ...
            if(resdata.data.ostyle == 0){
                $('.pay_order_type').html(getCookie('zh_choose')=='t'?'買漲':getCookie('zh_choose')=='e'?'lookup':getCookie('zh_choose')=='pt'?'Comprar':'买涨');
                $('.pay_order_type').addClass('rise');
                $('.pay_order_type').removeClass('fall');
            }else{
                $('.pay_order_type').html(getCookie('zh_choose')=='t'?'買跌':getCookie('zh_choose')=='e'?'lookdown':getCookie('zh_choose')=='pt'?'Compre ou caia':'买跌');
    ...
    $.each(resorderlist,function(k,v){
        ...
        if(_end_time >0){
            ...
            if(v.ostyle == 0){
                ...
                var ostyle_name = '<?php echo _lang("买涨")?>';
                if(chaprice >0){
                ...
                var ostyle_name = '<?php echo _lang("买跌")?>';
            ...
            const lang_cookie = getLangCookie('zh_choose')
            let title = ''
            if (lang_cookie == 't') {
                switch (v.ptitle) {
                    case '美国指数':
